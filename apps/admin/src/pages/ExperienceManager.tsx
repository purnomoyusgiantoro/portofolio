...
export const ExperienceManager: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editId) {
      await supabase.from('experience').update({ title, company, period, description }).eq('id', editId);
    } else {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-admin-text mb-1">Pengalaman Kerja</h1>
          <p className="text-sm text-admin-text-muted">Kelola riwayat pekerjaan Anda. Drag ⠿ untuk mengatur urutan.</p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-admin-border">
              {loading ? (
                ...
                <tr><td colSpan={4} className="px-6 py-8 text-center text-admin-text-muted">Belum ada data pengalaman</td></tr>
              ) : (
                items.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      <p className="text-admin-text-muted text-xs mt-1">{item.company}</p>
                    </td>
                    <td className="px-6 py-4 text-admin-text-muted">{item.period}</td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in overflow-y-auto">
          <div className="bg-admin-surface border border-admin-border rounded-2xl w-full max-w-xl overflow-hidden shadow-2xl my-8">
            <div className="px-6 py-4 border-b border-admin-border flex justify-between items-center bg-admin-bg/50">
              <h2 className="text-lg font-bold text-admin-text">{isEditing ? 'Edit Experience' : 'Add Experience'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Jabatan / Posisi" value={title} onChange={e => setTitle(e.target.value)} required placeholder="Misal: Senior Developer" />
                <FormField label="Nama Perusahaan" value={company} onChange={e => setCompany(e.target.value)} required placeholder="Misal: PT Teknologi..." />
              </div>
              <FormField label="Periode" value={period} onChange={e => setPeriod(e.target.value)} required placeholder="Misal: 2020 - Sekarang" />
              <FormField label="Deskripsi Pekerjaan" as="textarea" value={description} onChange={e => setDescription(e.target.value)} required />
      ...
      <ConfirmModal isOpen={!!deleteId} title="Hapus Pengalaman" message="Apakah Anda yakin ingin menghapus data pengalaman ini?" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} loading={deleteLoading} />
    </div>
  );
};
